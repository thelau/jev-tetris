...
// ─────────────────────────────────────────────────────────────────────────────
// v2: the documented shape of the API rather than its plainest form.
//
// Three changes, each targeting a failure mode TypeSafe documents for jev-1.13:
//   · instructions become objects with `question`, `focus`, `inspect` — `focus`
//     states the exact condition and what to ignore (mitigates literal reading),
//     `inspect` names the part of state the question is about (mitigates
//     indirection).
//   · criteria are per question. v1 gave every option all five facts while each
//     question asked about one, so four fifths of each option was irrelevant
//     detail — their documented distractor failure — and the criteria did not
//     match the instruction, which they list separately as "contradictory
//     instructions and criteria".
//   · state is nested and named, so `inspect` can point into it.
//
// It is also about four times cheaper, because the criteria repeated across the
// five questions are now short.

function seatPhrase(ev) {
  if (ev.freed > 0) return 'It frees space that was trapped before.'
  if (ev.sealsExisting === 0) return 'It rests clear of any space that was already trapped.'
  if (ev.sealsExisting <= 2) return 'It piles on top of space that was already trapped, pushing it further out of reach.'
  return 'It piles on top of a great deal of already-trapped space, putting it far out of reach.'
}

const CLAUSE = {
  sealed: (ev) => `${holesPhrase(ev.holesCreated)}.`,
  clears: (ev) => `${linesPhrase(ev.cleared)}.`,
  unburied: (ev) => seatPhrase(ev),
  low: (ev) => `${heightPhrase(ev.maxHeight)}.`,
  flat: (ev) => `${surfacePhrase(ev.bumpinessDelta)}.${wellPhrase(ev)}`,
}

export const VOICES_V2 = {
  sealed: {
    weight: 0.55,
    question: 'Which option traps the least empty space underneath the blocks?',
    focus: 'An empty square with a block above it can never be reached again. Prefer the option that traps none. Ignore how many rows are cleared and how tall the stack becomes.',
    inspect: 'board.buried',
  },
  clears: {
    weight: 0.22,
    question: 'Which option removes the most complete rows?',
    focus: 'A complete row disappears and everything above it falls. Prefer the option that removes the most. Ignore everything else about the board.',
    inspect: 'board.surface',
  },
  unburied: {
    weight: 0.15,
    question: 'Which option does least to bury space that is already trapped?',
    focus: 'Space already trapped can still be freed later by clearing the rows above it. Prefer options that rest clear of it, and avoid piling more on top. This is about existing trapped space only, not new trapped space.',
    inspect: 'board.buried',
  },
  low: {
    weight: 0.05,
    question: 'Which option leaves the stack furthest from the top?',
    focus: 'The game ends when the stack reaches the top. Prefer the option that keeps it lowest. Ignore rows cleared and trapped space.',
    inspect: 'board.room',
  },
  flat: {
    weight: 0.03,
    question: 'Which option leaves the most even and level surface?',
    focus: 'An even surface accepts more shapes later. Prefer flat over stepped, and stepped over jagged. Ignore rows cleared and trapped space.',
    inspect: 'board.shape',
  },
}

export function describeBoardV2(board, current, next) {
  const flat = describeBoard(board, current, next)
  return {
    board: { shape: flat.shape, surface: flat.surface, buried: flat.buried, room: flat.room },
    piece: { now: flat.piece.replace('The piece to place now is ', '').replace(/\.$/, ''),
             next: next ? flat.next.replace('The piece after this one is ', '').replace(/\.$/, '') : undefined },
  }
}

// One criteria set per question, carrying only what that question asks about.
export function describePlacementsV2(board, placements) {
  const evaluated = []
  const perVoice = Object.fromEntries(Object.keys(VOICES_V2).map((n) => [n, {}]))
  for (const p of placements) {
    const ev = evaluate(board, p)
    evaluated.push({ placement: p, ev })
    const seat = `Placed at ${where(p)}, ${orientation(p.type, p.rot)}.`
    for (const name of Object.keys(VOICES_V2)) {
      perVoice[name][p.id] = { what: `${CLAUSE[name](ev)} ${seat}` }
    }
  }
  return { perVoice, evaluated }
}

export function buildQuestionsV2(perVoice) {
  const qs = {}
  for (const [name, v] of Object.entries(VOICES_V2)) {
    qs[name] = {
      type: 'choice',
      instructions: { question: v.question, focus: v.focus, inspect: v.inspect },
      criteria: perVoice[name],
    }
  }
  qs.danger = {
    type: 'score',
    instructions: {
      question: 'How much trouble is this board in?',
      focus: 'Judge the board as it stands, before the next piece is placed.',
      inspect: 'board',
    },
    criteria: [
      { summary: 'Safe and tidy', signals: ['nothing trapped underneath', 'plenty of room above', 'level surface'] },
      { summary: 'Workable but untidy', signals: ['a little trapped space', 'an uneven surface'] },
      { summary: 'Struggling', signals: ['several trapped pockets', 'the stack well above halfway'] },
      { summary: 'About to lose', signals: ['almost no room left', 'many trapped pockets'] },
    ],
  }
  qs.doomed = {
    type: 'noul',
    instructions: {
      question: 'Is this board beyond saving?',
      focus: 'True only if no sequence of pieces could realistically recover it before it reaches the top.',
      inspect: 'board',
    },
  }
  return qs
